...
plt_A_phasic <- qplot(data=EDA_A_smoothed,x = Timestamp, y=phasic_values,geom="line") +
  ...
  ggtitle("Extracted Phasic Values from GSR Measurement - Participant 1") +
...
plt_F_phasic <- qplot(data=EDA_F_smoothed,x = Timestamp, y=phasic_values,geom="line") +
  xlab("Time") +
  ylab("Phasic Values [µS]") +
  ggtitle("Extracted Phasic Values from GSR Measurement - Participant 2")+
  geom_hline(yintercept = 0.01, linetype = "dotted", color = "red", size = 0.5) 
...
mapview(acc_buffer, zcol = "AccidentSeverityCategory_en", layer.name = "Accident Severity")
```

```{r}
tmap_mode("plot")

severity_colors <- c("Accident with fatalities" = "red", "Accident with light injuries" = "yellow", "Accident with property damage" = "blue", "Accident with severe injuries" = "orange")

acc_fatalities <- acc_buffer[acc_buffer$AccidentSeverityCategory_en == "Accident with fatalities", ]

# Create separate layers for different severity categories
fatalities_layer <- tm_shape(acc_fatalities) +
  tm_dots(col = "red", size = 0.25)


track_acc <- tm_shape(zh_districts_intersect) +
  tm_polygons() +
tm_shape(fluss) +
  tm_lines(col = "lightblue1", lwd = 10) +
tm_shape(edges_intersect) +
  tm_lines() +
tm_shape(acc_buffer) +
  tm_dots(col = "AccidentSeverityCategory_en", size = 0.25, palette = severity_colors, title = "Accident Severity") +
fatalities_layer +
tm_layout(title = "Cycling Track and Accident Severity", title.position = c("center", "top"), 
          inner.margins = c(0.1, 0.1, 0.1, 0.1),
          title.size = 1, 
          frame = F) +
tm_legend(position = c("center", "bottom"), 
                        bg.color = TRUE) 

track_acc
tmap_save(track_acc, filename = "plots/acc_map.png", width = 10, height = 8, units = "in")
...
The fact that both datasets are recorded along a single bike track within a predefined buffer indeed implies a certain level of spatial correlation / spatial clustering or association between these points. This arrangement suggests that they share a spatial context and are likely related due to their proximity within this specific area.
...
anova_plt <- ggplot(nn_A, aes(x = AccidentSeverityCategory_en, y = SCR_phasic)) +
...
png(file = "plots/anova.png", width = 800, height = 600)
