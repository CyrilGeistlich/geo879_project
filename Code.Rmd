...
peak_positions <- which(EDA_A_smoothed$phasic_values > peaksize)
peak_timestamps <- EDA_A_smoothed$Timestamp[peak_positions]
...
# Add one hour -> time difference
# track_sf <- track_sf |> 
#   mutate(ts_POSIXct = ts_POSIXct + hours(1))

peak_data$Timestamp <- as.POSIXct(peak_data$Timestamp)
track_sf$ts_POSIXct <- as.POSIXct(track_sf$ts_POSIXct)

expanded_track <- track %>%
...
peak_data$Timestamp <- round(peak_data$Timestamp, units = "secs")
track$ts_POSIXct <- round(track$ts_POSIXct, units = "secs")


peaks_merged <- left_join(peak_data, expanded_track, by = c("Timestamp" = "ts_POSIXct"))
...
# Assuming 'zh_districts' and 'full_merged_sf' contain necessary spatial and phasic_values columns
