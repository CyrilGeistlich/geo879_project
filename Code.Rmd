...
To predict the accident severity from phasic SCR values, we perform a linear regression model. Since we have categorical severity classes rather than continuous numerical values, we use dummy coding which leads to a table called contrast matrix. Generally, a categorical variable with n levels will be transformed into n-1 variables each with two levels. These n-1 new variables contain the same information than the single variable. This recoding creates a table called contrast matrix. By default option in R is to use the first level of the factor as a reference and interpret the remaining levels relative to this level.

Using multinomial logistic regression (one predictor and response variable with multiple categories)

```{r Categorical regression analysis}
# Convert AccidentSeverityCategory_en into factor
res <- model.matrix(~AccidentSeverityCategory_en, data = nn_A)
head(res[, -1])

library(nnet)

# Fit multinomial logistic regression model
model <- multinom(AccidentSeverityCategory_en ~ SCR_phasic, data = nn_A)

# Summary of the model
summary(model)

```
Using SCR phasic values as predictor variables to explain the categorical response variable bike accident severity (with 4 different levels) --> we try to understand if changes in the SCR phasic component are associated with changes in the categorical levels of accident severity.


--> Try K-Nearest Neighbors (KNN): This method predicts the classification of a point based on the classes of its nearest neighbors in the feature space.
